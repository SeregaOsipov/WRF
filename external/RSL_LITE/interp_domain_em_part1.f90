...
msize = (234 + ((num_wetscav_frcing - PARAM_FIRST_SCALAR + 1)) & 
 ...
 + ((num_irr_diag_mozart_mosaic_4bin_aq - PARAM_FIRST_SCALAR + 1)) )* nlev + 490
...
DO k = ckds,(ckde-1)
xv(k)= grid%qlsink(pig,k,pjg)
ENDDO
CALL rsl_lite_to_child_msg((((ckde-1))-(ckds)+1)*4,xv)
ENDIF
IF ( SIZE(grid%precr) .GT. 1 ) THEN 
DO k = ckds,(ckde-1)
xv(k)= grid%precr(pig,k,pjg)
ENDDO
CALL rsl_lite_to_child_msg((((ckde-1))-(ckds)+1)*4,xv)
ENDIF
IF ( SIZE(grid%preci) .GT. 1 ) THEN 
DO k = ckds,(ckde-1)
xv(k)= grid%preci(pig,k,pjg)
ENDDO
CALL rsl_lite_to_child_msg((((ckde-1))-(ckds)+1)*4,xv)
ENDIF
IF ( SIZE(grid%precs) .GT. 1 ) THEN 
DO k = ckds,(ckde-1)
xv(k)= grid%precs(pig,k,pjg)
ENDDO
CALL rsl_lite_to_child_msg((((ckde-1))-(ckds)+1)*4,xv)
ENDIF
IF ( SIZE(grid%precg) .GT. 1 ) THEN 
DO k = ckds,(ckde-1)
xv(k)= grid%precg(pig,k,pjg)
ENDDO
CALL rsl_lite_to_child_msg((((ckde-1))-(ckds)+1)*4,xv)
ENDIF
IF ( SIZE(grid%dust_flux) .GT. 1 ) THEN 
xv(1)=grid%dust_flux(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%seas_flux) .GT. 1 ) THEN 
xv(1)=grid%seas_flux(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%actnh3) .GT. 1 ) THEN 
DO k = 1,12
xv(k)= grid%actnh3(pig,k,pjg)
ENDDO
CALL rsl_lite_to_child_msg(((12)-(1)+1)*4,xv)
ENDIF
IF ( SIZE(grid%agrisoil_nh3) .GT. 1 ) THEN 
xv(1)=grid%agrisoil_nh3(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%fertilizer_nh3) .GT. 1 ) THEN 
DO k = 1,12
xv(k)= grid%fertilizer_nh3(pig,k,pjg)
ENDDO
CALL rsl_lite_to_child_msg(((12)-(1)+1)*4,xv)
ENDIF
IF ( SIZE(grid%freeinten_nh3) .GT. 1 ) THEN 
xv(1)=grid%freeinten_nh3(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%graze_nh3) .GT. 1 ) THEN 
xv(1)=grid%graze_nh3(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%industry_nh3) .GT. 1 ) THEN 
xv(1)=grid%industry_nh3(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%residential_nh3) .GT. 1 ) THEN 
xv(1)=grid%residential_nh3(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%transport_nh3) .GT. 1 ) THEN 
xv(1)=grid%transport_nh3(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%erod_dri) .GT. 1 ) THEN 
DO k = 1,config_flags%erosion_dim
xv(k)= grid%erod_dri(pig,pjg,k)
ENDDO
CALL rsl_lite_to_child_msg(((config_flags%erosion_dim)-(1)+1)*4,xv)
ENDIF
IF ( SIZE(grid%wd_so4_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_so4_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_no3_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_no3_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_nh4_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_nh4_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_oa_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_oa_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_so2_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_so2_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_sulf_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_sulf_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_hno3_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_hno3_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_nh3_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_nh3_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_asoa_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_asoa_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_bsoa_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_bsoa_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_cvasoa_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_cvasoa_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_cvbsoa_sc) .GT. 1 ) THEN 
xv(1)=grid%wd_cvbsoa_sc(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_so4_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_so4_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_no3_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_no3_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_nh4_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_nh4_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_oa_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_oa_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_so2_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_so2_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_sulf_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_sulf_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_hno3_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_hno3_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_nh3_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_nh3_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_asoa_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_asoa_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_bsoa_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_bsoa_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_cvasoa_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_cvasoa_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
IF ( SIZE(grid%wd_cvbsoa_cu) .GT. 1 ) THEN 
xv(1)=grid%wd_cvbsoa_cu(pig,pjg)
CALL rsl_lite_to_child_msg(4,xv)
ENDIF
DO itrace =  PARAM_FIRST_SCALAR, num_chem
DO k = ckds,(ckde-1)
xv(k)= chem(pig,k,pjg,itrace)
ENDDO
CALL rsl_lite_to_child_msg((((ckde-1))-(ckds)+1)*4,xv)
ENDDO
