...
CALL rsl_lite_from_parent_msg(((12)-(1)+1)*4,xv)
DO k = 1,12
grid%actnh3(pig,k,pjg) = xv(k)
ENDDO
ENDIF
IF ( SIZE(grid%agrisoil_nh3) .GT. 1 ) THEN 
CALL rsl_lite_from_parent_msg(4,xv)
grid%agrisoil_nh3(pig,pjg) = xv(1)
ENDIF
IF ( SIZE(grid%fertilizer_nh3) .GT. 1 ) THEN 
CALL rsl_lite_from_parent_msg(((12)-(1)+1)*4,xv)
DO k = 1,12
grid%fertilizer_nh3(pig,k,pjg) = xv(k)
ENDDO
ENDIF
IF ( SIZE(grid%freeinten_nh3) .GT. 1 ) THEN 
CALL rsl_lite_from_parent_msg(4,xv)
grid%freeinten_nh3(pig,pjg) = xv(1)
ENDIF
IF ( SIZE(grid%graze_nh3) .GT. 1 ) THEN 
CALL rsl_lite_from_parent_msg(4,xv)
grid%graze_nh3(pig,pjg) = xv(1)
ENDIF
IF ( SIZE(grid%industry_nh3) .GT. 1 ) THEN 
CALL rsl_lite_from_parent_msg(4,xv)
grid%industry_nh3(pig,pjg) = xv(1)
ENDIF
IF ( SIZE(grid%residential_nh3) .GT. 1 ) THEN 
CALL rsl_lite_from_parent_msg(4,xv)
grid%residential_nh3(pig,pjg) = xv(1)
ENDIF
IF ( SIZE(grid%transport_nh3) .GT. 1 ) THEN 
CALL rsl_lite_from_parent_msg(4,xv)
grid%transport_nh3(pig,pjg) = xv(1)
ENDIF
...
CALL bdy_interp (  &         
                 ...
                 cips, cipe, ckps, MIN( (ckde-1), ckpe ), cjps, cjpe,   &         
                  ngrid%chem(ngrid%sm31,ngrid%sm32,ngrid%sm33,itrace),  &   
...
,chem_bxs(cjms,1,1,itrace) ,ngrid%chem_bxs(njms,1,1,itrace) &
,chem_bxe(cjms,1,1,itrace) ,ngrid%chem_bxe(njms,1,1,itrace) &
,chem_bys(cims,1,1,itrace) ,ngrid%chem_bys(nims,1,1,itrace) &
,chem_bye(cims,1,1,itrace) ,ngrid%chem_bye(nims,1,1,itrace) &
,chem_btxs(cjms,1,1,itrace) ,ngrid%chem_btxs(njms,1,1,itrace) &
,chem_btxe(cjms,1,1,itrace) ,ngrid%chem_btxe(njms,1,1,itrace) &
,chem_btys(cims,1,1,itrace) ,ngrid%chem_btys(nims,1,1,itrace) &
,chem_btye(cims,1,1,itrace) ,ngrid%chem_btye(nims,1,1,itrace) &
,grid%dt,ngrid%dt&
                  ) 
ENDIF
ENDDO
